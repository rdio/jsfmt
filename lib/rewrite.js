  ...
  switch (pattern.type) {
    ...
    case 'FunctionExpression':
      ...
      if (pattern.rest && isWildcard(pattern.rest)) {
        wildcards['...' + pattern.rest.name] = node.params.slice(node.params.length - 1);
...
function rewriteNode(wildcards, replacement, node) {
  switch (replacement.type) {
    ...
    case 'Program':
    case 'BlockStatement':
      var length = Math.max(replacement.body.length, node.body.length);
      var result = [];
      for (var i = 0; i < length; i++) {
        if (i < replacement.body.length) {
          result.push(rewriteNode(wildcards, replacement.body[i], i < node.body.length ? node.body[i] : replacement.body[i]));
        } else {
          result.push(node.body[i]);
        }
      }
