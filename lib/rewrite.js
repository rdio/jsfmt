...
function match(wildcards, pattern, node) {
  ...
  // Treat Program and BlockStatements as synonymous
  if (_.isArray(pattern) && _.contains(['Program', 'BlockStatement'], node.type)) {
    return partial(wildcards, pattern, node.body);
  }
    ...
    default:
      if (_.isArray(replacement)) {
        for (var i = 0; i < replacement.length; i++) {
          replacement[i] = replaceWildcards(wildcards, replacement[i]);
        }
        replacement = {
          type: 'Program',
          body: replacement
